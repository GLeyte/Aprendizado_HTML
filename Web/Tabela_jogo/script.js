...
function adicionaJogador(){
  ...
  var jogador = {nome: nomeJogador, vitorias: 0, empates: 0, derrotas: 0, pontos: 0}
...
function adicionaPartida(){
  ...
  var jogador1 = document.querySelector(".from");
  var jogador2 = document.querySelector(".from");
...
$(document).on('click','#novoJogador',function(){
  ...
  $('#jogador1').append('<option value="'+newitemnum+'">'+newitemdesc+'</option>');
  
  // Set the select value with new option
  $("#jogador1").val(newitemnum);
  
  // Append the option to select
  $('#jogador2').append('<option value="'+newitemnum+'">'+newitemdesc+'</option>');
